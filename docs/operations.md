...
`vector-sys-fd-lens` is centered on systems programming work in R. The technical target is: Build an R toolkit that studies fd behavior through negative fixtures, with human-readable error snapshots and no credentials or hosted services.
...
The score is deterministic. The current threshold is 175. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
